...
<script>
import { updateSystemUserItem } from '@/api/systemUser'
...
export default {
    ...
    submitForm () {
      this.$refs.formData.validate((valid) => {
        if (valid) {
          ...
          updateSystemUserItem(this.formData).then(() => {
  ...
  watch: {
    ...
    itemData (val) {
      ...
      if (this.formData.userType === '3') {
        this.formData.shopId = [this.formData.headId, this.formData.branchId]
        this.handleItemChange(this.formData.shopId)
      }
    },
    headOptions (val) {
      this.headData = val
